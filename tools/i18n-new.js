...
const EN = path.join(I18N_DIR, 'en.js');

// Lowercase base subtag (da, de, fr), optionally a region (pt-br).
function isValidCode(c) { return /^[a-z]{2,3}(-[a-z0-9]+)?$/.test(c); }
...
function create(rawCode) {
  ...
  const dest = path.join(I18N_DIR, code + '.js');
  ...
  const out = fs.readFileSync(EN, 'utf8')
...
if (argCode) {
  ...
  (function ask() {
    rl.question('New language code (e.g. da, de, fr): ', function (answer) {
      answer = String(answer || '').trim().toLowerCase();
      if (!isValidCode(answer) || answer === 'en') {
